...
(leaf *org-mode
  ...
  (leaf org
    ...
    (defun org:setup-appearance ()
      ...
      (advice-add #'org-eldoc-documentation-function :before-until
                  (defun org-eldoc:display-link (&rest _)
    ...
    (defun org:setup-capture ()
      ...
      (advice-add 'org-capture-expand-file :filter-args
                  (defun org-capture:expand-var-file (file)
    ...
    (defun org:setup-export ()
      ...
      (advice-add #'org-export-to-file :around
                  (defun org-export:dont--trigger-save-hooks-on-export (orig-fn &rest args)
    ...
    (defun org:setup-hacks ()
      ...
      (advice-add 'org-get-agenda-file-buffer :around
                  (defun org:exclude-agenda-from-recentf (orig-fn file)
                    (let ((recentf-exclude (list (lambda (_file) t))))
                      (funcall orig-fn file))))

      (advice-add 'org-display-inline-images :around
                  (defun org:fix--inline-images-imagmagick (orig-fn &rest args)
  ...
  (leaf org-clock
    ...
    (defun org-clock:lazy-load ()
      "Lazy load org-clock until we actually use the commands."
      (org-clock-load))
    :pre-setq
    `((org-clock-persist-file . ,(concat my-etc-dir "org-clock-save.el")))
    :custom
    (org-clock-persist . 'history)
    (org-clock-in-resume . t)
    :advice
    (:before org-clock-in org-clock:lazy-load)
    (:before org-clock-out org-clock:lazy-load)
    (:before org-clock-in-last org-clock:lazy-load)
    (:before org-clock-goto org-clock:lazy-load)
    (:before org-clock-cancel org-clock:lazy-load)
    :config
...
If you're looking to only define an abbrev for a specific file/mode, see `skeleton:file-snip'."
    (declare (debug t))
